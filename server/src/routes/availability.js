...
router.post('/:username/:dayOfTheWeek', bodyParser, async(req, res) => {
    try {
        const newavailability = await AvailabilityService.createAvail(req.params.username, req.params.dayOfTheWeek);
...
// UPDATE DAY OF THE WEEK AVAILABILITY
router.patch('/:username/:dayOfTheWeek', bodyParser, async (req, res) => {
    try {
        const newavailability = await AvailabilityService.updateAvail(req.params.username, req.params.dayOfTheWeek);
        res.status(201).json(newavailability)
    }catch (err) {
        if (err instanceof CustomError)
            res.status(err.code).json({message: err.message})
...
router.delete('/:username/:dayOfTheWeek', (req, res) => {
    try {
        const availability = AvailabilityService.deleteAvail(req.params.id)
        res.status(200).json(availability)
    }catch (err){
        if (err instanceof CustomError)
            res.status(err.code).json({message: err.message})
